    ...
    extends JpaRepository<SsoUser, Integer>
{
    ...
    @Query(nativeQuery = true, value = "SELECT COUNT(*) FROM sso_user u WHERE u.is_delete = 0 AND (u.role_type = ?1 OR 0 = ?1) AND (u.status = ?2 OR -1 = ?2) AND (u.real_name LIKE CONCAT(CONCAT('%',?3),'%') OR u.user_name LIKE CONCAT(CONCAT('%',?3),'%') ) ")
    int findCount(byte roleType, byte status, String keyword);

    //@Query("SELECT u FROM SsoUser u WHERE u.isDelete = 0 AND (u.roleType = ?1 OR 0 = ?1)")
    //List<SsoUser> find(byte roleType);
    @Query(nativeQuery = true, value = "SELECT u.* FROM sso_user u WHERE u.is_delete = 0 AND (u.role_type = ?3 OR 0 = ?3) AND (u.status = ?4 OR -1 = ?4) AND (u.real_name LIKE CONCAT(CONCAT('%',?5),'%') OR u.user_name LIKE CONCAT(CONCAT('%',?5),'%') ) LIMIT ?2 OFFSET ?1 ")
    List<SsoUser> find(int offset, int limit, byte roleType, byte status, String keyword);
