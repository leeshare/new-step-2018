...
class ModalSearchLecture extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: { PageIndex: 1, PageSize: 5, Keyword: '', Status: 1, CourseWareType: 8, ApplicableScopes: (props.ApplicableScopes || '') },
    ...
    },
    {
        ...
        render: (text, record) => (
            <span>
                <a onClick={() => { this.props.callback(8, record) }}>选择</a>
            </span>
        ),
    }];
    ...
    render() {
        ...
        switch (this.state.editMode) {
            case "View":
                block_content = <Card title={'查看讲稿'} extra={<a onClick={() => { this.onLookView('Manage', null) }}>返回列表</a>}>
                    <div className='mobile-screen course-tabs'><CourseWare unitInfo={{ ...this.state.currentDataModel, Type: 8 }} /></div>
                </Card>
                break;
            case "Manage":
            default:
...
//表单组件 封装
const WrappedModalSearchChant = Form.create()(ModalSearchLecture);
