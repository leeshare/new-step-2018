...
class ModalSearchReader extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: { PageIndex: 1, PageSize: 5, Keyword: '', Status: 1, CourseWareType: 6, ApplicableScopes: (props.ApplicableScopes || '') },
    ...
    },
    {
        ...
        render: (text, record) => (
            <span>
                <a onClick={() => { this.props.callback(6, record) }}>选择</a>
            </span>
        ),
    }];
    ...
    getFields() {
        ...
        children.push(
            <Col span={8}>
                <FormItem {...formItemLayout} label={'朗读名称'} >
                    {getFieldDecorator('Keyword', { initialValue: '' })(
                        <Input placeholder="朗读名称、关键字" />
                    )}
                </FormItem>
            </Col>
        );
    ...
    render() {
        ...
        switch (this.state.editMode) {
            case "View":
                block_content = <Card title={'查看朗读'} extra={<a onClick={() => { this.onLookView('Manage', null) }}>返回列表</a>}>
                    <div className='mobile-screen course-tabs'><CourseWare unitInfo={{ ...this.state.currentDataModel, Type: 6 }} /></div>
                </Card>
                break;
            case "Manage":
            default:
...
//表单组件 封装
const WrappedModalSearchReader = Form.create()(ModalSearchReader);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedModalSearchReader);