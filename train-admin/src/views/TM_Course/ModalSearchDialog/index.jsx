...
class ModalSearchDialog extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: { PageIndex: 1, PageSize: 5, Keyword: '', Status: 1, CourseWareType: 2, ApplicableScopes: (props.ApplicableScopes || '') },
    ...
    columns = [
        ...
        },
        {
            ...
            render: (text, record) => (
                <span>
                    <a onClick={() => { this.props.callback(2, record) }}>选择</a>
                </span>
            ),
        }];
    ...
    getFields() {
        ...
        children.push(
            <Col span={8}>
                <FormItem {...formItemLayout} label={'对话'} >
                    {getFieldDecorator('Keyword', { initialValue: '' })(
                        <Input placeholder="对话内容、知识点、关键字" />
                    )}
                </FormItem>
            </Col>
        );
    ...
    render() {
        ...
        switch (this.state.editMode) {
            case "View":
                block_content = <Card title={'查看对话'} extra={<a onClick={() => { this.onLookView('Manage', null) }}>返回列表</a>}>
                    <div className='mobile-screen course-tabs'><CourseWare unitInfo={{ ...this.state.currentDataModel, Type: 2 }} /></div>
                </Card>
                break;
            case "Manage":
            default:
...
//表单组件 封装
const WrappedModalSearchDialog = Form.create()(ModalSearchDialog);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedModalSearchDialog);