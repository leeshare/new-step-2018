...
class ModalSearchTalk extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: { PageIndex: 1, PageSize: 5, Keyword: (props.Keyword || ''), Status: 1, CourseWareType: 7, ApplicableScopes: (props.ApplicableScopes || '') },
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.Keyword != this.props.Keyword || nextProps.ApplicableScopes != this.props.ApplicableScopes) {
            this.setState({ pagingSearch: { ...this.state.pagingSearch, Keyword: nextProps.Keyword || '', ApplicableScopes: nextProps.ApplicableScopes || '' } })
            setTimeout(() => {
                this.onSearch();
            }, 100)
        }
    ...
    columns = [{
        ...
        render: (text, record, index) => {
            return <a onClick={() => { this.onLookView('View', record) }}>{record.Knowledges}</a>
        }
    },
    {
        title: '例句',
    ...
    },
    {
        ...
        render: (text, record) => (
            <span>
                <a onClick={() => { this.props.callback(7, record) }}>选择</a>
            </span>
        ),
    }];
    ...
    render() {
        ...
        switch (this.state.editMode) {
            case "View":
                block_content = <Card title={'查看看图说话'} extra={<a onClick={() => { this.onLookView('Manage', null) }}>返回列表</a>}>
                    <div className='mobile-screen course-tabs'><CourseWare unitInfo={{ Talks: [...this.state.currentDataModel], Type: 7 }} /></div>
                </Card>
                break;
            case "Manage":
            default:
...
//表单组件 封装
const WrappedModalSearchTalk = Form.create()(ModalSearchTalk);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedModalSearchTalk);