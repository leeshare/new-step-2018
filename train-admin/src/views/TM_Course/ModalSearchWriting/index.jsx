...
class ModalSearchwriting extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: { PageIndex: 1, PageSize: 5, Keyword: '', Status: 1, CourseWareType: 5, ApplicableScopes: (props.ApplicableScopes || '') },
    ...
    },
    {
        ...
        render: (text, record) => (
            <span>
                <a onClick={() => { this.props.callback(5, record) }}>选择</a>
            </span>
        ),
    }];
    ...
    getFields() {
        ...
        children.push(
            <Col span={8}>
                <FormItem {...formItemLayout} label={'听写内容'} >
                    {getFieldDecorator('Keyword', { initialValue: '' })(
                        <Input placeholder="听写内容" />
                    )}
                </FormItem>
            </Col>
        );
    ...
    render() {
        ...
        switch (this.state.editMode) {
            case "View":
                block_content = <Card title={'查看听写'} extra={<a onClick={() => { this.onLookView('Manage', null) }}>返回列表</a>}>
                    <div className='mobile-screen course-tabs'><CourseWare unitInfo={{ ...this.state.currentDataModel, Type: 5 }} /></div>
                </Card>
                break;
            case "Manage":
            default:
...
//表单组件 封装
const WrappedModalSearchwriting = Form.create()(ModalSearchwriting);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedModalSearchwriting);