...
import { message, Spin, Modal, Form, Row, Col, Input, Select, Button, Icon, Table, Pagination, Card, Tabs, Carousel, Timeline, InputNumber, Tag } from 'antd';

import './index.less'
import { getDictionaryTitle, getViewEditModeTitle, getDictionaryTitleByCode } from '@/utils';
import { getCourseUnitList } from '@/actions/tm';
...
class CourseView extends React.Component {
    ...
    renderEditModeOfView() {
        ...
        switch (this.props.editMode) {
                    ...
                    <Form>
                        ...
                        >
                            {getFieldDecorator('CoverPath', {
                                initialValue: this.state.dataModel.Cover,
                                rules: [{
                                    required: true, message: '请上传任务封面图!',
                                }],
                            })(
                                <ImageUpload />
                                )}
                        ...
                        <FormItem
                            {...formItemLayout}
                            label="任务名称"
                            extra="如果维护多个相同的任务类型，任务名称建议追加1,2来区别"
                        >
                        ...
                        {this.props.editMode == 'Create' ? <FormItem
                            {...formItemLayout}
                            label="任务类型"
                            extra="添加后不允许修改，当前设置将影响学习任务设计中的可选单元类型。"
                        >
                            {getFieldDecorator('SupportUnitType', {
                                rules: [{
                                    required: true, message: '请设置任务类型!',
                                }]
                            })(
                                <Select><Option value={'0'}>综合</Option>
                                    {this.props.dic_courseWareTypes.map((item) => {
                            ...
                            extra=""
                        >
                                <span className="ant-form-text">{this.state.dataModel.SupportUnitType == 0 ? '综合' : getDictionaryTitleByCode(this.props.dic_courseWareTypes, this.state.dataModel.SupportUnitType)}</span>
                            </FormItem>}
                        <FormItem
                            {...formItemLayout}
                            label="关键字"
                            extra="建议设置课程名称，便于检索一组数据"
                        >
                        ...
                        <FormItem
                            ...
                            label="显示顺序"
                        >
                            {getFieldDecorator('OrderNo', { initialValue: this.state.dataModel.OrderNo })(
                        ...
                        <FormItem
                            ...
                            label="金币数"
                        >
                            {getFieldDecorator('Golds', { initialValue: this.state.dataModel.Golds })(
                        ...
                        }
                        {
                            ...
                            >
                                {getFieldDecorator('AuditRemark', {
                                    rules: [{
                                        required: true, message: '请对本次学习任务修改进行说明!',
                                    }]
                                })(
                                    <TextArea rows={4} />
                                    )}
            ...
            case "View":
            case "Delete":
                let block_content_tab1 = (
                    <Form>
                        <FormItem
                            {...formItemLayout}
                            label="任务封面"
                        >
                            <img src={this.state.dataModel.Cover} />
                        </FormItem>
                        <FormItem
                            {...formItemLayout}
                            label="任务名称"
                        ...
                        <FormItem
                            ...
                            label="金币数"
                        >
                            <span className="ant-form-text">{this.state.dataModel.Golds}</span>
                        </FormItem>
                        <FormItem
                            {...formItemLayout}
                            label="任务状态"
                ...
                let block_content_tab2 = (
                    <Row type="flex" justify="center" gutter={16}>
                        ...
                        <Col span={8}>
                            <Timeline>
                                {this.state.courseUnits.map((unitInfo, index) => {
                                    return <Timeline.Item color={this.state.currentUnitIndex == index ? 'red' : 'green'}><a onClick={() => { this.switchUnit(index) }}>{this.getUnitTitle(unitInfo, index)}</a></Timeline.Item>
                                })}
                            </Timeline>
                        </Col>
                    </Row>
                ...
                block_content = (
                    <div className={'course-tabs'}>
                        ...
                        <Tabs defaultActiveKey="tab1">
                            ...
                            <TabPane tab="学习单元" key="tab2">{block_content_tab2}</TabPane>
                        </Tabs>
                    </div>
                )
                break;
            ...
            <Card title={title} extra={
                <div>
                    {this.props.editMode == 'Design' && <div className='course-designer-control'>
                        <Button style={{ marginRight: 10 }} type="dashed" icon="plus" onClick={() => { this.onAddUnit('Cover') }}>单元封面</Button>
                        {
                            this.props.dic_courseWareTypes
                                .filter((item) => {
                                    if (this.state.dataModel.SupportUnitType == 0) {
