...
import { seachBaseLibrary } from '@/actions/tm';
import { getDictionaryTitle } from '@/utils';
import CourseWare from '../CourseWare';
const FormItem = Form.Item;
class ModalSearchBaseLibrary extends React.Component {
    LibraryTypeName = "";
    constructor(props) {
        super(props)
        this.state = {
        ...
        if (props.LibraryType == 1) {
            this.LibraryTypeName = "字";
        }
        else {
            this.LibraryTypeName = "词句";
        }
    ...
    //table 输出列定义
    columns = [{
        title: `${this.LibraryTypeName}名称`,
        //自定义显示
        render: (text, record, index) => {
            return <a onClick={() => { this.onLookView('View', record) }}>{record.Name} {this.props.LibraryType==1?record.Phonetic:''}</a>
        }
    },
    {
        title: '适用对象',
    ...
    getFields() {
        ...
        children.push(
            <Col span={8}>
                <FormItem {...formItemLayout} label={`${this.LibraryTypeName}名称`} >
                    {getFieldDecorator('Keyword', { initialValue: this.props.Keyword })(
                        <Input placeholder={`${this.LibraryTypeName}或关键字`} />
                    )}
                </FormItem>
            </Col>
        );
    ...
    render() {
        ...
        switch (this.state.editMode) {
            case "View":
                block_content = <Card title={`查看${this.LibraryTypeName}`} extra={<a onClick={() => { this.onLookView('Manage', null) }}>返回列表</a>}>
                    <div className='mobile-screen course-tabs'><CourseWare unitInfo={{ ...this.state.currentDataModel, Type: type }} /></div>
                </Card>
                break;
            case "Manage":
            default:
...
//表单组件 封装
const WrappedModalSearchBaseLibrary = Form.create()(ModalSearchBaseLibrary);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedModalSearchBaseLibrary);