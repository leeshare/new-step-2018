...
//业务接口方法引入
import { getLinkInfoList, saveLinkInfo, deleteLinkInfo } from '@/actions/other';
//业务数据视图（增、删、改、查)
import LinkInfoView from '../LinkInfoView/index.js';
class LinkInfoManage extends React.Component {
    constructor(props) {
        super(props)
    ...
    columns = [
        ...
        },
        {
            ...
            render: (text, record, index) => {
                return record.LinkInfoType == 1 ? '调查问卷' : '广告';
            }
        },
        {
            title: YSI18n.get('Url地址'),
            width: 200,//可预知的数据长度，请设定固定宽度
            dataIndex: 'LinkPath',
        },
        {
    ...
    fetch = (pagingSearch) => {
        ...
        this.props.getLinkInfoList({ type: 2 }).payload.promise.then((response) => {
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                ...
                case "Edit": //提交
                    let entity = {
                        LinkInfoID: dataModel.LinkInfoID,
                        LinkInfoType: dataModel.LinkInfoType,
                        ImagePath: dataModel.FormImagePaths_Temp[0].value,
                        LinkPath: dataModel.LinkPath,
                        Status: dataModel.Status,
                    }
                    this.props.saveLinkInfo(entity).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                ...
                case "Delete":
                    //提交
                    this.props.deleteLinkInfo({ id: dataModel.LinkInfoID }).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "View":
            case "Delete":
                block_content = <LinkInfoView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
                        ...
                        >
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    <ButtonGroup>
                                        <Button onClick={() => { this.onLookView('Create', {}) }} icon="plus">{YSI18n.get('广告')}</Button>
                                    </ButtonGroup>
                                </Col>
                            </Row>
                        </Form>
...
//表单组件 封装
const WrappedDictionaryManage = Form.create()(LinkInfoManage);
...
function mapDispatchToProps(dispatch) {
    return {
        //各业务接口
        getLinkInfoList: bindActionCreators(getLinkInfoList, dispatch),
        saveLinkInfo: bindActionCreators(saveLinkInfo, dispatch),
        deleteLinkInfo: bindActionCreators(deleteLinkInfo, dispatch),
    };
}
//redux 组件 封装
