...
//业务接口方法引入
import { importStudent } from '@/actions/file';
...
class StudentImportTemplet extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      editMode: '',
    ...
    (this: any).onSaveStudent = this.onSaveStudent.bind(this);
  }
  componentWillMount() {
  }

  onSaveStudent(url: string) {
    this.setState({ loading: true })
    this.props.importStudent({ filePath: url }).payload.promise.then((response) => {
  ...
  /*onExport = () => {
    ...
    var url = `${env.serverURL}/edu/courseArrangeImport/errCourseExports?token=${env.getToken()}`;
  ...
  render() {
    ...
    var url = serverURL + "/excelTemplate/学生导入模板.xls";
            ...
            <Form className="search-form" enctype="multipart/form-data" method="post">
              <Row type='flex' justify='start' align='middle' gutter={24}>
                <Col span={8}>
                  ...
                  {/*}<FileDownloader
                    apiUrl={'/sso/getExcelTemplet'}//api下载地址
                    method={'post'}//提交方式
                    //options={{ filePath: 'upload/importTemplate/courseUpload.xlsx' }}
                    options={{ filePath: 'excelTemplate/学生导入模板.xlsx' }}
                    title={'下载上传模板'}
                  >
                  </FileDownloader>*/}
                </Col>
...
//表单组件 封装
const WrappedManage = Form.create()(StudentImportTemplet);
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    importStudent: bindActionCreators(importStudent, dispatch),
  };
}
//redux 组件 封装
