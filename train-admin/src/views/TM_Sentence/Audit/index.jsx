...
import { seachBaseLibrary, auditSentenceInfo, switchSentenceInfoStatus, deleteSentenceInfo } from '@/actions/tm';
import { getDictionaryTitle } from '@/utils';
import SentenceView from '../View';
const FormItem = Form.Item;
class SentenceAudit extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand:false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Status: "-1", AuditStatus: "1", LibraryType: 2, ApplicableScopes: '-1' },
    ...
    columns = [
        {
            ...
            render: (text, record, index) => {
                return <a onClick={() => { this.onLookView('View', record) }}>{record.Chinese}</a>
            }
        },
        {
            title: `英文`,
            //自定义显示
            render: (text, record, index) => {
                return <span>{(record.English || '').slice(0, 100)}</span>
            }
        },
        {
            title: '适用对象',
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                ...
                case "Audit": 
                    //提交
                    this.props.auditSentenceInfo({...this.state.currentDataModel,...dataModel}).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    onSwitchStatus(item) {
        //提交
        this.props.switchSentenceInfoStatus({ ...item, Status: 1 - item.Status }).payload.promise.then((response) => {
            let data = response.payload.data;
            if (data.result === false) {
                message.error(data.message);
            }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "Delete":
            case "Audit":
                block_content = <SentenceView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
...
//表单组件 封装
const WrappedSentenceAudit = Form.create()(SentenceAudit);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        auditSentenceInfo: bindActionCreators(auditSentenceInfo, dispatch),
        switchSentenceInfoStatus: bindActionCreators(switchSentenceInfoStatus, dispatch),
        deleteSentenceInfo: bindActionCreators(deleteSentenceInfo, dispatch)
    };
}
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedSentenceAudit);