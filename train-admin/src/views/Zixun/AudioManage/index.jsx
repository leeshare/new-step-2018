...
//业务数据视图（增、删、改、查)
import InfoView from '../AudioInfoView';
import './index.less';
...
class ZixunAudioManage extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: { pageIndex: 1, pageSize: 20, Keyword: '', Status: -1, InfoType: 3, channelIds: [-2], PublishBeginDate: null, PublishEndDate: null },
            data_list: [],
            data_list_total: 0,
            loading: false,
            channel_data_List: [],
    ...
    getChannelList() {
        this.props.zixun_ChannelList({ isZb: 1 }).payload.promise.then((response) => {
            ...
            if (data.result === false) {
                ...
                this.setState({ channel_data_List: data.data_list })
            }
        })
    }
    //table 输出列定义
    columns = [{
        ...
        render: (text, record, index) => {
            ...
            let count_view = record.status == 1 ? <div className='dv_block_status'>
                <span className='iconitem'><Icon type="user" className='icon' />{record.readCount}</span>
                <span className='iconitem'><Icon type="heart" className='icon' />{record.collectCount}</span>
                <a onClick={() => { this.onLookView('Comment', record) }}><span className='iconitem'><Icon type="message" className='icon' />{record.evaluationCount}</span></a>
            </div> : null;
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Comment":
                block_content = <CommentView viewCallback={this.onViewCallback} {...this.state} />
                break;
                case "Create":
                case "Edit": //提交
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'center' }}>
                                    <ButtonGroup>
                                        ...
                                        <Button onClick={() => { this.onLookView('Create', { title: '', content: '', describe:'', coverType: 1, covers: [], isRecommend: 0, isTop: 0, topNum: 0, channel_data_List: this.state.channel_data_List, channelList: [] }) }} icon="plus">{YSI18n.get('资讯')}</Button>
                                    </ButtonGroup>
                                </Col>
                            </Row>
                        </Form>
...
//表单组件 封装
const WrappedRoleManage = Form.create()(ZixunAudioManage);
