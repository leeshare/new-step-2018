...
//业务数据视图（增、删、改、查)
import InfoView from '../InfoView/index.js';
...
class ZixunManage extends React.Component {
        ...
        render: (text, record, index) => {
            ...
            let count_view = record.status == 1 ? <div className='dv_block_status'>
                <span className='iconitem'>
                    <Icon type="user" className='icon' />{record.readCount}</span>
                <span className='iconitem'><Icon type="heart" className='icon' />{record.collectCount}</span>
                <a onClick={() => { this.onLookView('Comment', record) }}><span className='iconitem'>
                    <Icon type="message" className='icon' />{record.evaluationCount}</span>
                </a>
            </div> : null;
...
//表单组件 封装
const WrappedRoleManage = Form.create()(ZixunManage);
