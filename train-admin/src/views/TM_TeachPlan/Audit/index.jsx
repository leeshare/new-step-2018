...
import { getTeachPlanList, auditTeachPlanInfo, switchTeachPlanInfo } from '@/actions/tm';
import { getDictionaryTitle, convertTextToHtml } from '@/utils';
import TeachPlanView from '../View';
import TeachPlanDetailAudit from '../TeachPlanDetail/audit';
...
class TeachPlanManage extends React.Component {
    ...
    columns = [
        ...
        },
        {
            ...
            //自定义显示
            render: (text, record, index) => {
                return <a onClick={() => { this.onLookView('ManageCourse', record) }}>{record.TotalTimes}</a>
            }
        },
        {
            title: '课程',
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.auditTeachPlanInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                ...
                case "Audit":
                    //提交
                    this.props.auditTeachPlanInfo({ ...this.state.currentDataModel, ...dataModel }).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                ...
                case "Delete":
                    //提交
                    this.props.switchTeachPlanInfo(this.state.currentDataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    onSwitchStatus(item) {
        //提交
        this.props.switchTeachPlanInfo({ ...item, Status: 1 - item.Status }).payload.promise.then((response) => {
            let data = response.payload.data;
            if (data.result === false) {
                message.error(data.message);
            }
        ...
        switch (this.state.editMode) {
            case "ManageCourse":
                block_content = <Card title={'课次安排管理'} extra={<a onClick={() => { this.onViewCallback() }}><Icon type="rollback" />返回</a>}>
                    <TeachPlanDetailAudit {...this.state.currentDataModel} />
                </Card>
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        auditTeachPlanInfo: bindActionCreators(auditTeachPlanInfo, dispatch),
        switchTeachPlanInfo: bindActionCreators(switchTeachPlanInfo, dispatch)
    };
}
//redux 组件 封装
