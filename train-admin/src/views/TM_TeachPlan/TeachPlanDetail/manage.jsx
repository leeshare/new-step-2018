...
class TeachPlanDetailManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    columns = [
        ...
        },
        {
            ...
            render: (text, record) => (
                <span>
                    <a onClick={() => { this.onLookView('Create', { IsInsertAdd: true, TeachPlanID: this.props.TeachPlanID, IsStandand: 1, TeachPlanNo: record.TeachPlanNo }) }}>{'前插入课次安排'}</a>                    
                    <span className="ant-divider" />
                    <a onClick={() => { this.onLookView('Edit', record) }}>{'编辑'}</a>
                    <span className="ant-divider" />
                    {record.AuditStatus == 3 ? <a disabled>删除</a> : <a onClick={() => { this.onDelete(record) }}>删除</a>}
                </span>
            ),
        }
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => {
                                        this.onLookView("Create", { TeachPlanID: this.props.TeachPlanID, IsStandand: 1, TeachPlanNo: this.state.data_list_total + 1 })
                                    }} icon="plus">课次安排</Button>
                                </Col></Row>
                        </Form>
...
//表单组件 封装
const WrappedTeachPlanDetailManage = Form.create()(TeachPlanDetailManage);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedTeachPlanDetailManage);