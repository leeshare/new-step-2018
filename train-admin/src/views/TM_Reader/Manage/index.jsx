...
import { searchCourseWare, saveCourseWare_ReaderInfo, deleteCourseWareInfo } from '@/actions/tm';
...
class ReaderManage extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Knowledges: '', Status: "-1", AuditStatus: "-1", CourseWareType: 6, ApplicableScopes: "-1" },
            dic_ApplicableScopes: [],
            dic_Status: [],
            dic_AuditStatus: [],
            dic_SwitchStatus: [],
            dic_WritingObjectTypes: [],
            dic_ReaderLayoutTypes: [],
            data_list: [],
            data_list_total: 0,
            loading: false
        };
    ...
    columns = [
        {
            ...
            render: (text, record, index) => {
                return <a onClick={() => { this.onLookView('View', record) }}>{ellipsisText(record.CourseWareName, 20)}</a>
            }
        },
        {
            title: `朗读字数`,
            //自定义显示
            render: (text, record, index) => {
                return <span>{record.WordCount}</span>
            }
        },
        {
            title: `朗读时长（秒）`,
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveCourseWare_ReaderInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "View":
            case "Delete":
                block_content = <ReaderView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', { Content: '', ContentLayout: 1, IsShowPhonetic: 1 }) }} icon="plus">朗读</Button>
...
//表单组件 封装
const WrappedReaderManage = Form.create()(ReaderManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveCourseWare_ReaderInfo: bindActionCreators(saveCourseWare_ReaderInfo, dispatch),
        deleteCourseWareInfo: bindActionCreators(deleteCourseWareInfo, dispatch)
    };
}
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedReaderManage);