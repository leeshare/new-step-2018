...
import { searchCourseWare, saveCourseWare_PatternInfo, deleteCourseWareInfo } from '@/actions/tm';
import { getDictionaryTitle, ellipsisText } from '@/utils';
import PatternView from '../View';
const FormItem = Form.Item;
class PatternManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Knowledges: '', Status: "-1", AuditStatus: "-1", CourseWareType: 4, ApplicableScopes: "-1" },
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveCourseWare_PatternInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "View":
            case "Delete":
                block_content = <PatternView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', {}) }} icon="plus">句型</Button>
...
//表单组件 封装
const WrappedPatternManage = Form.create()(PatternManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveCourseWare_PatternInfo: bindActionCreators(saveCourseWare_PatternInfo, dispatch),
        deleteCourseWareInfo: bindActionCreators(deleteCourseWareInfo, dispatch)
    };
}
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedPatternManage);