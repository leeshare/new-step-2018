...
import TeachClassScheduleQuery from '../Query'
import './index.less'
const TabPane = Tabs.TabPane;
const FormItem = Form.Item;
class TeachClassScheduleOrgQuery extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
            ...
            <Tabs defaultActiveKey={'tab0'} type="card">
                {
                    this.state.data_list.map((item, index) => {
                        return <TabPane tab={item.OrganizationShortName} key={`tab${index}`}>
                            <TeachClassScheduleQuery adminMode={1} OrganizationID={item.OrganizationID} />
                        </TabPane>
                    })
                }
            </Tabs>
...
//表单组件 封装
const WrappedTeachClassScheduleQuery = Form.create()(TeachClassScheduleOrgQuery);
