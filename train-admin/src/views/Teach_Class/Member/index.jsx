...
import { getTeachClassList } from '@/actions/teach';
import { getDictionaryTitle } from '@/utils';
import TeachClassView from '../View';
import TeachClassMemberView from './view.jsx';
const FormItem = Form.Item;
class TeachClassMemberManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Status: "-1", Teacher: '', CourseSpecialty: '', CourseLevel: '', PeriodType: '', BeginDate: '', EndDate: '' },
            dic_Status: [],
    ...
    columns = [
        {
            ...
            render: (text, record, index) => {
                return <a onClick={() => { this.onLookView('View', record) }}>{record.TeachClassName}</a>
            }
        },
        {
            title: '班级成员',
            ...
            render: (text, record, index) => {
                return `${record.ClassStudents}`;
                //return `${record.ClassStudents}/${record.MaxStudents == 0 ? '不限' : record.MaxStudents}`;
            }
        },
        {
            title: '班级类型',
            width: 100,//可预知的数据长度，请设定固定宽度
            render: (text, record, index) => {
                return `${getDictionaryTitle(this.state.dic_PeriodTypes, record.PeriodType)}`;
            }
        },
        {
            title: '上课方式',
        ...
        },
        {
            ...
            width: 100,//可预知的数据长度，请设定固定宽度
            render: (text, record, index) => {
                return `${record.Duration}${getDictionaryTitle(this.state.dic_DurationTypes, record.DurationType)}`;
            }
        },
        {
            title: '上课教师',
        ...
        },
        {
            ...
            render: (text, record) => (
                <span>
                    <a onClick={() => { this.onLookView('MemberList', record) }}>管理学员</a>
                </span>
            ),
        }];
    ...
    render() {
        ...
        switch (this.state.editMode) {
            case "MemberList":
                block_content = <TeachClassMemberView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "View":
                block_content = <TeachClassView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    {/* <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', {}) }} icon="plus">新增班级</Button> */}
...
//表单组件 封装
const WrappedTeachClassMemberManage = Form.create()(TeachClassMemberManage);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedTeachClassMemberManage);