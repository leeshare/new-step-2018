...
class SSO_AgentManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Status: "-1", OrganizationType: 4 },
    ...
    getFields() {
        ...
        children.push(
            <Col span={8}>
                <FormItem {...formItemLayout} label={'招生代理'} >
                    {getFieldDecorator('Keyword', { initialValue: this.state.pagingSearch.Keyword })(
                        <Input placeholder={'招生代理编号或招生代理简称或招生代理全称'} />
                    )}
                </FormItem>
            </Col>
        );
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveOrganizationInfo({ ...dataModel, OrganizationType: 4 }).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', {}) }} icon="plus">添加招生代理</Button>
...
//表单组件 封装
const WrappedSSO_AgentManage = Form.create()(SSO_AgentManage);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedSSO_AgentManage);