...
import { seachBaseLibrary, auditCharacterInfo, switchCharacterInfoStatus, deleteCharacterInfo } from '@/actions/tm';
import { getDictionaryTitle } from '@/utils';
import CharacterView from '../View';
const FormItem = Form.Item;
class CharacterAudit extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand:false,
    ...
    columns = [
        {
            ...
            render: (text, record, index) => {
                return <a onClick={() => { this.onLookView('View', record) }}>{record.Word}</a>
            }
        },
        {
            title: `音标`,
            //自定义显示
            render: (text, record, index) => {
                return <span>{record.Phonetic}</span>
            }
        },
        {
            title: `英文`,
            //自定义显示
            render: (text, record, index) => {
                return <span>{(record.Word_En || '').slice(0, 100)}</span>
            }
        },
        {
            title: '适用对象',
    ...
    onSearch = (e) => {
        ...
        this.props.form.validateFields((err, values) => {
            ...
            this.props.seachBaseLibrary(pagingSearch).payload.promise.then((response) => {
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveCharacterInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                ...
                case "Delete":
                    //提交
                    this.props.deleteCharacterInfo(this.state.currentDataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                ...
                case "Audit":
                    //提交
                    this.props.auditCharacterInfo({...this.state.currentDataModel,...dataModel}).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    onSwitchStatus(item) {
        //提交
        this.props.switchCharacterInfoStatus({ ...item, Status: 1 - item.Status }).payload.promise.then((response) => {
            let data = response.payload.data;
            if (data.result === false) {
                message.error(data.message);
            }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "Delete":
            case "Audit":
                block_content = <CharacterView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
...
//表单组件 封装
const WrappedCharacterAudit = Form.create()(CharacterAudit);
...
function mapDispatchToProps(dispatch) {
    return {
        seachBaseLibrary: bindActionCreators(seachBaseLibrary, dispatch),
        auditCharacterInfo: bindActionCreators(auditCharacterInfo, dispatch),
        switchCharacterInfoStatus: bindActionCreators(switchCharacterInfoStatus, dispatch),
        deleteCharacterInfo: bindActionCreators(deleteCharacterInfo, dispatch)
    };
}
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedCharacterAudit);