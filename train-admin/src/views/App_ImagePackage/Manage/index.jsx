...
import { getAppImagePackageList, saveAppImagePackageInfo, deleteAppImagePackageInfo } from '@/actions/app';
import { getDictionaryTitle } from '@/utils';
import ImagePackageView from '../View';
const FormItem = Form.Item;
class ImagePackageManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveAppImagePackageInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                ...
                case "Delete":
                    //提交
                    this.props.deleteAppImagePackageInfo(this.state.currentDataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
...
//表单组件 封装
const WrappedLiveManage = Form.create()(ImagePackageManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveAppImagePackageInfo: bindActionCreators(saveAppImagePackageInfo, dispatch),
        deleteAppImagePackageInfo: bindActionCreators(deleteAppImagePackageInfo, dispatch)
    };
}
//redux 组件 封装
