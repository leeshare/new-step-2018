...
class CharacterManage extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Status: "-1", ResourceType: "-1", IncludeResourceVersionTypes: [] },
            dic_resourceTypes: [],
            dic_Status: [],
    ...
    columns = [
        {
            ...
            render: (text, record, index) => {
                return <a onClick={() => { this.onLookView('View', record) }}>{record.ResourceName}</a>
            }
        },
        {
            title: `关键词`,
        ...
        },
        {
            ...
            render: (text, record, index) => {
                return getDictionaryTitle(this.state.dic_resourceTypes, record.ResourceType);
            }
        },
        // {
        //     title: '其它版本',
        //     width: 100,//可预知的数据长度，请设定固定宽度
        //     render: (text, record, index) => {
        //         return <a onClick={() => { this.onLookView('View', record) }}>{record.ResourceVersions}</a>
        //     }
        // },
        {
            title: '状态',
        ...
        children.push(
            <Col span={8}>
                <FormItem
                    ...
                    label="资源版本"
                >
                    {getFieldDecorator('IncludeResourceVersionTypes', { initialValue: this.state.pagingSearch.IncludeResourceVersionTypes })(
                        <Select mode="multiple">
                            <Option value="-1">全部</Option>
                            {this.state.dic_resourceVersionTypes.map((item) => {
                                return <Option value={item.value}>{item.title}</Option>
                            })}
                        </Select>
                    )}
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveTMResourceInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "View":
            case "Delete":
                block_content = <ResourceView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveTMResourceInfo: bindActionCreators(saveTMResourceInfo, dispatch),
    };
}
//redux 组件 封装
