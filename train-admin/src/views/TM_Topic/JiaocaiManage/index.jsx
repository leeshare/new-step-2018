...
import { getTopicList, saveCourseTopicInfo, deleteCourseTopicInfo } from '@/actions/tm';
import { getDictionaryTitle } from '@/utils';
import JiaocaiView from '../View/jiaocaiIndex';
import CourseGroupManage from '../CourseGroup';
const FormItem = Form.Item;
class TopicManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Status: "-1", AuditStatus: "-1", TopicType: 'CT01', IsJiaocai: true, ParentID: '' },
            dic_TopicTypes: [],
            dic_Status: [],
            dic_AuditStatus: [],
            data_list: [],
    ...
    columns = [
        ...
        },
        {
            ...
            //自定义显示
            render: (text, record, index) => {
                return <a onClick={() => { this.onLookView('ManageCourseGroup', record) }}>{record.Courses}</a>
            }
        },
        {
            title: '显示顺序',
    ...
    getFields() {
        ...
        children.push(
            <Col span={8}>
                <FormItem {...formItemLayout} label={'教材'} >
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveCourseTopicInfo({ ...dataModel, TopicType: 'CT01', ParentID: '' }).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            case "ManageCourseGroup":
                block_content = <Card title={'教材课程'} extra={<a onClick={() => { this.onViewCallback() }}><Icon type="rollback" />返回</a>}>
                    <CourseGroupManage {...this.state.currentDataModel} ParentID={this.state.currentDataModel.TopicID} />
                </Card>
            ...
            case "View":
            case "Delete":
                block_content = <JiaocaiView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', {}) }} icon="plus">教材</Button>
...
//表单组件 封装
const WrappedTopicManage = Form.create()(TopicManage);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedTopicManage);