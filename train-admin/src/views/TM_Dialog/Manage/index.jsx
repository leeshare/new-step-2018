...
import { searchCourseWare, saveCourseWare_DialogInfo, deleteCourseWareInfo } from '@/actions/tm';
import { getDictionaryTitle, ellipsisText } from '@/utils';
import DialogView from '../View';
const FormItem = Form.Item;
class DialogManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Knowledges: '', Status: "-1", AuditStatus: "-1", CourseWareType: 2, ApplicableScopes: "-1" },
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveCourseWare_DialogInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "View":
            case "Delete":
                block_content = <DialogView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', {}) }} icon="plus">对话</Button>
...
//表单组件 封装
const WrappedDialogManage = Form.create()(DialogManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveCourseWare_DialogInfo: bindActionCreators(saveCourseWare_DialogInfo, dispatch),
        deleteCourseWareInfo: bindActionCreators(deleteCourseWareInfo, dispatch)
    };
}
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedDialogManage);