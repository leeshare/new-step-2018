...
class HqManage extends React.Component {
  constructor(props) {
    super(props);
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        let block_opertion = this.state.curr_auth ?
          ...
          <Popover placement="bottom" title={null} content={popmenu2} trigger={record.f_type != 'folder' ? "click" : ''}>
            <Icon type='ellipsis' className='ico_ellipsis' />
          </Popover>
        {/*<ButtonGroup>
  ...
  fetch = (pagingSearch) => {
    ...
    this.props.folderShareListQuery({ orgId: '00000000-0000-0000-0000-000000000001' }).payload.promise.then((response) => {
                  ...
                  {this.state.menus.length > 0 ?
                    <div>
                      ...
                      {/*for(var i = 0; i < this.state.menus.length; i++){
                                        var m = this.state.menus[i];
                                        if(i == this.state.menus.length - 1){
                                          return <span>{m.name}</span>
                                        }else {
                                          return <a onClick={() => {this.gotoMenu(m.id)}}>{m.name}</a>
                                        }
...
//表单组件 封装
const WrappedShareManage = Form.create()(HqManage);
