...
import { searchCourseWare, saveCourseWare_TalkInfo, deleteCourseWareInfo } from '@/actions/tm';
import { getDictionaryTitle, ellipsisText } from '@/utils';
import TalkView from '../View';
const FormItem = Form.Item;
class TalkManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Knowledges: '', Status: "-1", AuditStatus: "-1", CourseWareType: 7, ApplicableScopes: "-1" },
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveCourseWare_TalkInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', {}) }} icon="plus">看图说话</Button>
...
//表单组件 封装
const WrappedTalkManage = Form.create()(TalkManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveCourseWare_TalkInfo: bindActionCreators(saveCourseWare_TalkInfo, dispatch),
        deleteCourseWareInfo: bindActionCreators(deleteCourseWareInfo, dispatch)
    };
}
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedTalkManage);