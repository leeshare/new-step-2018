...
import { searchCourseWare, saveCourseWare_WritingInfo, deleteCourseWareInfo } from '@/actions/tm';
import { getDictionaryTitle, ellipsisText } from '@/utils';
import WritingView from '../View';
const FormItem = Form.Item;
class WritingManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand: false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Knowledges: '', Status: "-1", AuditStatus: "-1", CourseWareType: 5, ApplicableScopes: "-1" },
            dic_ApplicableScopes: [],
            dic_Status: [],
            dic_AuditStatus: [],
            dic_SwitchStatus:[],
            dic_WritingObjectTypes:[],
            data_list: [],
            data_list_total: 0,
            loading: false
        };
    ...
    columns = [
        {
            ...
            render: (text, record, index) => {
                return <a onClick={() => { this.onLookView('View', record) }}>{record.WritingContent}</a>
            }
        },
        // {
        //     title: `听写类型`,
        //     //自定义显示
        //     render: (text, record, index) => {
        //          return getDictionaryTitle(this.state.dic_WritingObjectTypes, record.WritingType);
        //     }
        // },
        {
            title: '适用对象',
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveCourseWare_WritingInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "View":
            case "Delete":
                block_content = <WritingView viewCallback={this.onViewCallback} {...this.state} />
                break;
            case "Manage":
            default:
                block_content = (
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', {}) }} icon="plus">听写</Button>
...
//表单组件 封装
const WrappedWritingManage = Form.create()(WritingManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveCourseWare_WritingInfo: bindActionCreators(saveCourseWare_WritingInfo, dispatch),
        deleteCourseWareInfo: bindActionCreators(deleteCourseWareInfo, dispatch)
    };
}
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedWritingManage);