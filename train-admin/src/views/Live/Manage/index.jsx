...
import { getLiveList, saveLiveInfo, deleteLiveInfo } from '@/actions/live';
        ...
        children.push(
            <Col span={8}>
                <FormItem
                    ...
                    label="直播进度"
                >
                    {getFieldDecorator('LiveProgress', { initialValue: this.state.pagingSearch.LiveProgress })(
                        <Select>
                            <Option value="-1">全部</Option>
                            {this.state.dic_LiveProgressStatus.map((item) => {
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveLiveInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8 }} onClick={() => { this.onLookView('Create', {}) }} icon="plus">直播计划</Button>
...
//表单组件 封装
const WrappedLiveManage = Form.create()(LiveManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveLiveInfo: bindActionCreators(saveLiveInfo, dispatch),
