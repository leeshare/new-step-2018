...
import { searchCourseWare, saveCourseWare_ChantInfo, deleteCourseWareInfo } from '@/actions/tm';
import { getDictionaryTitle,ellipsisText } from '@/utils';
import ChantView from '../View';
const FormItem = Form.Item;
class ChantManage extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            expand:false,
            ...
            pagingSearch: { PageIndex: 1, PageSize: 30, Keyword: '', Status: "-1", AuditStatus:"-1", CourseWareType: 3, ApplicableScopes: "-1"},
    ...
    columns = [
        ...
        },
        {
            ...
            //自定义显示
            render: (text, record, index) => {
                return <span>{ellipsisText(record.Content,30)}</span>
            }
        },
        {
            title: '适用对象',
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                case "Edit": //提交
                    this.props.saveCourseWare_ChantInfo(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.result === false) {
                            message.error(data.message);
                        }
                        ...
                        >
                            ...
                            <Row>
                                <Col span={24} style={{ textAlign: 'right' }}>
                                    ...
                                    <Button style={{ marginLeft: 8}} onClick={() => { this.onLookView('Create', {}) }} icon="plus">跟唱</Button>
...
//表单组件 封装
const WrappedChantManage = Form.create()(ChantManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        saveCourseWare_ChantInfo: bindActionCreators(saveCourseWare_ChantInfo, dispatch),
        deleteCourseWareInfo: bindActionCreators(deleteCourseWareInfo, dispatch)
    };
}
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedChantManage);