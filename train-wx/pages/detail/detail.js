...
Page({
  ...
  onShareAppMessage: function () {
    var that = this
    return {
      title: that.data.keywordObj.keyword,
      desc: "",
      path: '/pages/detail/detail?keyword=' + that.data.keywordObj.keyword
    }
  },
  onLoad: function (options) {
    let keywordObj = app.globalData.pushData
    let keyword = options.keyword
    if (!keywordObj || !keywordObj.keyword) {
      keywordObj = { keyword: keyword }
    }
    wx.setNavigationBarTitle({ title: keywordObj.keyword });
    this.setData({ keywordObj: keywordObj })
    var that = this;
    wx.getSystemInfo({
